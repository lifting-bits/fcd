...
struct DfsStackItem {
  ...
  block_iterator end() { return block.successors.end(); }
};

void ensureLoopsExit(PreAstContext& function) {
  // This ensures that every loop has an exit for the purpose of calculating the
  // post-dominator tree.
  for (auto iter = scc_begin(&function); iter != scc_end(&function); ++iter) {
    if (iter.hasLoop()) {
      bool hasOutsideSuccessor = false;
      SmallPtrSet<PreAstBasicBlock*, 16> loopMembers(iter->begin(),
                                                     iter->end());
      for (auto block : *iter) {
        hasOutsideSuccessor =
            any_of(block->successors, [&](PreAstBasicBlockEdge* edge) {
      ...
      if (!hasOutsideSuccessor) {
        for (auto block : *iter) {
          auto outsidePredecessorIter =
              find_if(block->predecessors, [&](PreAstBasicBlockEdge* edge) {
                return loopMembers.count(edge->from) == 0;
              });

          if (outsidePredecessorIter != block->predecessors.end()) {
            // Insert a fake edge going to a fake exit edge from any entry
            // block. This helps the post-dominator tree.
            PreAstBasicBlock& fakeExit = function.createBlock();
            PreAstBasicBlockEdge& fakeEdge = function.createEdge(
                *block, fakeExit, *function.getContext().expressionForFalse());
...
bool derefEqual(const Expression* a, const Expression* b) { return *a == *b; }
...
SmallVector<Expression*, 4> splitNaryOperator(
    Expression& expr, NAryOperatorExpression::NAryOperatorType type) {
...
Expression* createDisjunction(AstContext& ctx, TIter begin, TIter end) {
  // This removes trivially true expressions from disjunctions. For instance,
  // the following: (A) || (!A && B) || (!A && !B && C) should be simplified as
  // A || B || C. (The function relies on the format of the conditionals that
  // reaching conditions create: it's absolutely not a general-purpose
  ...
  for (auto iter = begin; iter != end; ++iter) {
    ...
    conjunction.expressions =
        splitNaryOperator(**iter, NAryOperatorExpression::ShortCircuitAnd);
  ...
  if (sizeOneConjunctions.size() > 0) {
    ...
    if (largerConjunctions.size() > 1) {
      ...
      // The simplest and most reasonable thing to do is th simplify for
      // expressions that are present in every remaining expressions. This is
      // not a perfect solution, but the problem is NP-complete, so yeah.
      ...
      if (commonSubExpressions.size() > 0) {
        ...
        resultExpressions.push_back(ctx.nary(
            NAryOperatorExpression::ShortCircuitAnd,
            commonSubExpressions.begin(), commonSubExpressions.end(), true));

        // Remove in reverse order to avoid invalidating indices that are still
        // used.
        sort(largerConjunctions.begin(), largerConjunctions.end());
        for (auto iter = largerConjunctions.rbegin();
             iter != largerConjunctions.rend(); ++iter) {
  ...
  for (auto& conjunction : inputConjunctions) {
    auto andedExpression = ctx.nary(NAryOperatorExpression::ShortCircuitAnd,
                                    conjunction.expressions.begin(),
                                    conjunction.expressions.end(), true);
    resultExpressions.push_back(andedExpression);
  }
  return ctx.nary(NAryOperatorExpression::ShortCircuitOr,
                  resultExpressions.begin(), resultExpressions.end(), true);
}
...
class Structurizer {
 ...
 private:
  ...
  bool isRegion(PreAstBasicBlock* entry, PreAstBasicBlock* exit) {
    ...
    // If the exit is the header of a loop that contains the entry, the
    // dominance frontier must only contain the exit.
    if (!domTree.dominates(entry, exit)) {
      bool onlyEntryOrExit =
          all_of(entrySuccessors, [=](PreAstBasicBlock* frontierBlock) {
    ...
    for (PreAstBasicBlock* entrySuccessor : entrySuccessors) {
      ...
      bool domFrontierNotCommon =
          any_of(entrySuccessor->predecessors, [&](PreAstBasicBlockEdge* edge) {
            return domTree.dominates(entry, edge->from) &&
                   !domTree.dominates(exit, edge->from);
          });
    ...
    for (PreAstBasicBlock* exitSuccessor : exitSuccessors) {
      if (domTree.properlyDominates(entry, exitSuccessor) &&
          exitSuccessor != exit) {
        return false;
      }
    }
  ...
  bool regionContains(PreAstBasicBlock* entry, PreAstBasicBlock* exit,
                      PreAstBasicBlock* block) {
    ...
    return domTree.dominates(entry, block) &&
           !(domTree.dominates(exit, block) && domTree.dominates(entry, exit));
  }

  StatementReference foldBasicBlocks(block_iterator begin, block_iterator end) {
    // Fold blocks into one sequence. This is easy now that we can just iterate
    // over the region range, which is sorted in post order.
    ...
    for (PreAstBasicBlock* bb : make_range(begin, end)) {
      // Identify back-edges. If we find any back-edge, we know that we have to
      // wrap this region in a loop and insert break statements.
      ...
      if (loopExit != nullptr) {
        for (PreAstBasicBlockEdge* exitingEdge : loopExit->predecessors) {
          ...
          if (memberBlocks.count(&predecessor) > 0) {
            Statement* conditionalBreak =
                ctx.breakStatement(exitingEdge->edgeCondition);
    ...
    for (PreAstBasicBlock* bb : make_range(begin, end)) {
      ...
      for (auto predEdge : bb->predecessors) {
        ...
        // already. This saves us from getting the entry condition for the
        // region's entry block (we don't want it because entry is
        // unconditional), and loop back-edges (the edge condition should be
        // applied to a break statement).
        if (predEdge->from == bb) {
          continue;
        }
        ...
        if (iter != reachingConditions.end()) {
          if (iter->second.size() == 0) {
            // The parent was reached unconditionally. It has no paths instead
            // of one path with true, so just insert one path with the reaching
            // condition (if it is not unconditonal itself).
            if (predEdge->edgeCondition != ctx.expressionForTrue()) {
              disjunction.push_back({predEdge->edgeCondition});
            }
          } else {
            auto startIter = disjunction.insert(
                disjunction.end(), iter->second.begin(), iter->second.end());
            if (predEdge->edgeCondition != ctx.expressionForTrue()) {
              for (auto appendIter = startIter; appendIter != disjunction.end();
                   ++appendIter) {
      ...
      if (disjunction.size() > 0) {
        ...
        for (++orIter; orIter != disjunction.end(); ++orIter) {
          auto prefixMismatch =
              mismatch(commonPrefix.begin(), commonPrefix.end(),
                       orIter->begin(), orIter->end(), derefEqual);
          commonPrefix.erase(prefixMismatch.first, commonPrefix.end());

          auto suffixMismatch =
              mismatch(commonSuffix.rbegin(), commonSuffix.rend(),
        ...
        // Nest into if statements for easy merging by the branch combining
        // pass.
        if (disjunctionTerms.size() > 0) {
          // (Some more post-processing for inverted prefixes happens here.)
          Expression* disjunctionExpression = createDisjunction(
              ctx, disjunctionTerms.rbegin(), disjunctionTerms.rend());
          statementToInsert = {ctx.ifElse(disjunctionExpression,
                                          move(statementToInsert).take())};
        }
        for (Expression* term :
             make_range(commonSuffix.rbegin(), commonSuffix.rend())) {
          statementToInsert = {
              ctx.ifElse(term, move(statementToInsert).take())};
        }
        for (Expression* term :
             make_range(commonPrefix.rbegin(), commonPrefix.rend())) {
          statementToInsert = {
              ctx.ifElse(term, move(statementToInsert).take())};
        }
      }
    ...
    if (isLoop) {
      resultSequence = {ctx.loop(ctx.expressionForTrue(),
                                 ...
                                 move(resultSequence).take())};
    }
    return resultSequence;
  }

  // This function splits a single region in up to 3 regions. The new regions
  // are: entry -> return.first return.first -> return.second return.second ->
  // exit
  StatementReference splitAndFoldRegion(block_iterator entry,
                                        block_iterator exit) {
    SmallPtrSet<PreAstBasicBlock*, 8> allBlocks;
    auto blocksEnd = blocksInReversePostOrder.end();
    if (exit == blocksEnd) {
      allBlocks.insert(blocksInReversePostOrder.begin(),
                       blocksInReversePostOrder.end());
    }
    ...
    while (!dfsStack.empty()) {
      ...
      // Do not traverse blocks that are outside of this region. This means that
      // we have to stop when we get to the exit. If the exit is the end
      // iterator, then we are in the presence of a loop, and we have to stop
      // whenever we see a block that hasn't been visited by the region
      // algorithm yet.
      ...
      auto edgeToIter = find_if(dfsStack, [&](DfsStackItem& item) {
        return &item.block == edge->to;
      });
    ...
    SmallVector<PreAstBasicBlockEdge*, 4> enteringEdges(backEdges.begin(),
                                                        backEdges.end());
    ...
    for (PreAstBasicBlockEdge*& edge : enteringEdges) {
      // The position of the new entry block in the basic block list is quite a
      // big deal. We don't need to care much about the position of the exit,
      // for instance, because it's pretty much guaranteed to be swallowed. The
      // entry, however, is returned.
      if (&edge != &enteringEdges.front() &&
          edge->to != enteringEdges.front()->to) {
        PreAstBasicBlock* newEntry =
            &function.createRedirectorBlock(enteringEdges);
        if (loopNodes.count(*entry) == 0) {
          // Insert new block before the first loop node.
          auto insertPosition =
              find_if(blocksInReversePostOrder, [&](PreAstBasicBlock* block) {
                return loopNodes.count(block) != 0;
              });
          loopEntry = blocksInReversePostOrder.insert(insertPosition, newEntry);
        } else {
          // This is weird: the entry block *has* to remain the block that was
          // first in blocksInReversePostOrder, because the collection has to be
          // in sync with the post-dominator tree (and any block that we create
          // at this point cannot be in the post-dominator tree). As a
          // consequence, to create a new entry, we need to swap the memory
          // contents (and edge references) of the block that we create with the
          // entry that we currently have. Insert the block after the first
          // entry.
          newEntry->swap(**entry);
    ...
    for (PreAstBasicBlockEdge*& edge : exitingEdges) {
      if (&edge != &exitingEdges.front() &&
          edge->to != exitingEdges.front()->to) {
        PreAstBasicBlock* exitBlock =
            &function.createRedirectorBlock(exitingEdges);
        loopExit = blocksInReversePostOrder.insert(
            find(blocksInReversePostOrder, exitingEdges.back()->from),
            exitBlock);
        break;
      }
    }
  ...
  bool reduceRegion(PreAstBasicBlock* exit) {
    ...
    if (regionSize == 1) {
      ...
      if (!any_of(entry->successors, [=](PreAstBasicBlockEdge* edge) {
    ...
    // Clear the successors of every block in that region. (We need to do it at
    // least on the entry node, and doing it on the other nodes help show better
    // graphs using PreAstContext::view().)
    for (PreAstBasicBlock* block :
         make_range(blocksInReversePostOrder.begin(), regionEnd)) {
      for (auto edge : block->successors) {
        auto eraseIter = remove(edge->to->predecessors.begin(),
                                edge->to->predecessors.end(), edge);
    ...
    if (exit != nullptr) {
      ...
      while (predIter != exit->predecessors.end()) {
        // This leaves unreachable nodes pointing to exit, but we're going to
        // get rid of the graph anyway.
      ...
      auto& newExitEdge =
          function.createEdge(*entry, *exit, *ctx.expressionForTrue());
 ...
 public:
  Structurizer(PreAstContext& function, DomTree& domTree,
               PostDomTree& postDomTree, DomFrontier& domFrontier)
...
AstBackEnd::AstBackEnd(void) : ModulePass(ID) {}
...
void AstBackEnd::getAnalysisUsage(llvm::AnalysisUsage& usage) const {
...
void AstBackEnd::runOnFunction(llvm::Function& func) {
  ...
  // Ensure that loops all have an exit node, for the sake of the post-dominator
  // tree.
  ...
  Structurizer structurizer(*blockGraph, domTree, postDomTree,
                            dominanceFrontier);
  node.getBody() = structurizer.structurizeFunction().take();
}

bool AstBackEnd::runOnModule(llvm::Module& module) {
...
AstBackEnd* createAstBackEnd(void) { return new AstBackEnd; }

static llvm::RegisterPass<AstBackEnd> ast_backend(
    "ast_backend", "Produce AST from LLVM module", true, false);